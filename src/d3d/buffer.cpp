...
{    
    ...
    if (data)
    {
        ...
        D3D_CHECK(d3d->device->CreateBuffer(&bufferDesc, &bufferSubData, &dxBuffer));
    }
    else
    {
        D3D_CHECK(d3d->device->CreateBuffer(&bufferDesc, nullptr, &dxBuffer));
    }
}
...
void* Buffer::Map()
{
    ...
    d3d->ctx->Map(dxBuffer.Get(), 0u, D3D11_MAP_WRITE_DISCARD, 0u, &mappedResource);
    return mappedResource.pData;
}

void Buffer::Unmap()
{
    d3d->ctx->Unmap(dxBuffer.Get(), 0u);
}
...
{
    d3d->ctx->IASetVertexBuffers(0u, 1u, dxBuffer.GetAddressOf(), &stride, &offset);
}

void IndexBuffer::Bind()
{
    d3d->ctx->IASetIndexBuffer(dxBuffer.Get(), DXGI_FORMAT_R32_UINT, 0u);
}

void ConstantBuffer::Bind()
{
    d3d->ctx->VSSetConstantBuffers(0u, 1u, dxBuffer.GetAddressOf());
}