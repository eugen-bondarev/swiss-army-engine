...
namespace VK
{
    RenderTarget::RenderTarget(const Vec2ui size, const std::vector<Ref<ImageView>>& imageViews, const RenderPass& renderPass, const size_t samples, const bool useDepth, const Device& device) : size {size}, renderPass {renderPass}, device {device}
    {
        ...
        if (useMultisample)
        {
            // multiSampleTexture = CreatePtr<Texture2D>(size, GetSwapChain().GetImageFormat(), VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_IMAGE_ASPECT_COLOR_BIT, SamplesToVKFlags(samples));
            multiSampleTexture = CreatePtr<Texture2D>(size, GetSwapChain().GetImageFormat(), VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_IMAGE_ASPECT_COLOR_BIT, SamplesToVKFlags(samples));
        }
