...
namespace VK
{
    Frame::Frame(int first_semaphore, int last_semaphore, int amount_of_semaphores) : firstSemaphore { first_semaphore }, lastSemaphore { last_semaphore }
    {
        ...
        {
            VK_TRY(vkCreateSemaphore(GetDevice()->GetVkDevice(), &semaphore_info, nullptr, &semaphores[i]));
        }
        
        VK_TRY(vkCreateFence(GetDevice()->GetVkDevice(), &fence_info, nullptr, &inFlightFence));
    }
    ...
    Frame::~Frame()
    {
        ...
        vkDestroyFence(GetDevice()->GetVkDevice(), inFlightFence, nullptr);
    }
    ...
    uint32_t FrameManager::AcquireSwapChainImage()
    {
        ...
        {
            vkWaitForFences(GetDevice()->GetVkDevice(), 1, &imagesInFlight[image_index], VK_TRUE, UINT64_MAX);
        }
        
        vkWaitForFences(GetDevice()->GetVkDevice(), 1, &frame->GetInFlightFence(), VK_TRUE, UINT64_MAX);
        vkResetFences(GetDevice()->GetVkDevice(), 1, &frame->GetInFlightFence());
    ...
    {			
        Frame* frame = GetCurrentFrame();
        GetSwapChain()->waitSemaphores = { frame->GetSemaphore(frame->lastSemaphore) };
        // GetSwapChain()->Present(0u, 0u);
        GetSwapChain()->Present1(&frame->GetSemaphore(frame->lastSemaphore), 1);
        NextFrame();
    }
