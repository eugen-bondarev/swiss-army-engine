...
namespace VK
{
    Texture2D::Texture2D(const Vec2ui size, const uint32_t numChannels, const void* data, const VkImageUsageFlags usageFlags, const Device& device) : device{device}, size{size}
    {
        ...
        image = CreatePtr<VK::Image>(size, VK_FORMAT_R8G8B8A8_UNORM, usageFlags, VK_SAMPLE_COUNT_1_BIT, device);
    ...
    {
        image = CreatePtr<VK::Image>(size, format, usageFlags, numSamples, device);
